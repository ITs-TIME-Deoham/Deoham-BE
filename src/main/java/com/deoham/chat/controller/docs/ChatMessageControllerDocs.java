...
public interface ChatMessageControllerDocs {
    ...
    ApiResponse<ChatMessagePageResponse> getMessages(
            @Parameter(description = "조회할 채팅방 UUID") UUID roomId,
            @Parameter(
                    description = "커서 — 이 시각 이전의 메시지를 조회합니다. ISO-8601 형식 (예: `2024-01-15T10:30:00Z`). 생략하면 최신 메시지부터 조회합니다.",
                    example = "2024-01-15T10:30:00Z"
            ) Instant before,
            @Parameter(description = "한 번에 조회할 메시지 수 (기본값: 30)", example = "30") int size);
    ...
    ApiResponse<ChatAttachmentPresignResponse> presignAttachment(
            @Parameter(description = "첨부파일을 업로드할 채팅방 UUID") UUID roomId,
            @Valid ChatAttachmentPresignRequest request);
}
